...
class Bookstore:
    ...
    def add_book(self,name,writer,price,count):
        query = "INSERT INTO books (title, author, price, quantity) VALUES (%s, %s, %s, %s)"
        values = (name, writer, price, count)
        self.cursor.execute(query, values)
        self.db.commit()
        tkinter.messagebox.showinfo(title="Book added", message="Book added successfully.")

    def sell_book(self,name, count):
        query_select = "SELECT quantity FROM books WHERE title = %s"
        self.cursor.execute(query_select, (name,))
        result = self.cursor.fetchone()

        if result and result[0] >= count:
            # Update the book inventory and add to the 'sales' table
            query_update = "UPDATE books SET quantity = quantity - %s WHERE title = %s"
            self.cursor.execute(query_update, (count, name))

            query_insert_sale = "INSERT INTO sales (title, quantity, sale_date) VALUES (%s, %s, %s)"
            sale_date = datetime.now()
            self.cursor.execute(query_insert_sale, (name, count, sale_date))

            self.db.commit()
            tkinter.messagebox.showinfo(title="Book added", message=f"Book sold successfully. Sale date: {sale_date}")
            # print(f"Book sold successfully. Sale date: {sale_date}")
    ...
    def search_book(self, keyword):
        ...
        if result:
            for book in result:
                tkinter.messagebox.showinfo(title="Book found", message= book)
        else:
            tkinter.messagebox.showwarning(title="Not Found", message= "there is no book or writer with " +keyword)

    def generate_sales_report(self, start_date, end_date):
        # Generate sales report for a specified time period
...
bookObj = Bookstore(host="localhost", user="root", password="", database="bookstore")
